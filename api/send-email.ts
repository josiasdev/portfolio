...
const translations: Record<string, {
  ...
  pt: {
    portfolioTitle: 'Josias Batista — Portfólio',
    subTitle: 'Nova mensagem de contato recebida',
    nameLabel: 'Nome:',
    emailLabel: 'Email:',
    subjectLabel: 'Assunto:',
    messageLabel: 'Mensagem:',
    footerText: 'Mensagem enviada através de',
    subjects: {
  ...
  en: {
    portfolioTitle: 'Josias Batista — Portfolio',
    subTitle: 'New contact message received',
    nameLabel: 'Name:',
    emailLabel: 'Email:',
    subjectLabel: 'Subject:',
    messageLabel: 'Message:',
    footerText: 'Message sent via',
    subjects: {
      project: 'Project Proposal',
      general: 'General Inquiry / Opportunity',
      feedback: 'Feedback / Suggestion',
      other: 'Other Topic',
    },
  },
  es: {
    portfolioTitle: 'Josias Batista — Portafolio',
    subTitle: 'Nuevo mensaje de contacto recibido',
    nameLabel: 'Nombre:',
    emailLabel: 'Email:',
    subjectLabel: 'Asunto:',
    messageLabel: 'Mensaje:',
    footerText: 'Mensaje enviado a través de',
    subjects: {
      project: 'Propuesta de Proyecto',
      general: 'Consulta General / Oportunidad',
...
export default async function handler(
  ...
  try {
    ...
    const { name, email, subject, message, language = 'pt' } = result.data;
    const langKey = (language in translations) ? language : 'pt';
    const dict = translations[langKey];

    const formattedSubject = dict.subjects[subject] || subject;
    const emailSubject = `[${dict.portfolioTitle.split(' — ')[1]}] ${formattedSubject} — ${name}`;
      ...
      html: `
        <div style="max-width: 600px; margin: 0 auto; font-family: 'Helvetica Neue', Arial, sans-serif; background-color: #f8fafc; border-radius: 8px; overflow: hidden; border: 1px solid #e2e8f0; color: #1e293b;">
          <div style="background-color: #0f172a; padding: 24px; border-bottom: 3px solid #14b8a6;">
            <h1 style="color: #ffffff; font-size: 18px; margin: 0; font-weight: 700;">${dict.portfolioTitle}</h1>
            <p style="color: #94a3b8; font-size: 12px; margin: 4px 0 0 0;">${dict.subTitle}</p>
          </div>
          
          <div style="padding: 24px; background-color: #ffffff;">
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
              <tr>
                <td style="padding: 8px 0; color: #64748b; width: 100px; font-weight: 600;">${dict.nameLabel}</td>
                <td style="padding: 8px 0; color: #0f172a; font-weight: 600;">${name}</td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #64748b; font-weight: 600;">${dict.emailLabel}</td>
                <td style="padding: 8px 0;"><a href="mailto:${email}" style="color: #0d9488; text-decoration: none; font-weight: 600;">${email}</a></td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #64748b; font-weight: 600;">${dict.subjectLabel}</td>
            ...
            <div style="margin-top: 20px; border-top: 1px solid #e2e8f0; padding-top: 16px;">
              <p style="color: #64748b; font-size: 12px; font-weight: 700; margin: 0 0 8px 0; text-transform: uppercase; letter-spacing: 0.5px;">${dict.messageLabel}</p>
          ...
          <div style="background-color: #f1f5f9; padding: 16px 24px; border-top: 1px solid #e2e8f0; text-align: center; font-size: 12px; color: #64748b;">
            <p style="margin: 0;">${dict.footerText} <a href="https://josias-batista-portfolio.vercel.app" style="color: #0d9488; text-decoration: none; font-weight: 600;">josias-batista-portfolio.vercel.app</a></p>
          </div>
        </div>
      `,
    });
